...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ["--help"],
            {"primers": None, "reference": None, "output": None, "verbose": False},
        ),
    ],
)
def test_get_args(arg_list: list[str], expected: dict[str, str]) -> None:
    ...
    for key in ["input", "output", "reference", "primers"]:
        expected[key] = os.path.abspath(expected[key])
